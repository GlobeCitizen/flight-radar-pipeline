...
def get_distance(lat1, lon1, lat2, lon2):
    ...
    lat1, lon1, lat2, lon2 = map(lambda x: math.radians(x), [lat1, lon1, lat2, lon2])
...
def create_flights_silver_df(flights: list, spark: SparkSession) -> DataFrame:
    ...
    flights_df = flights_df.withColumn(
        "time", F.from_unixtime("time", "yyyy-MM-dd HH:mm:ss")
    )
...
def flights_enriched_df(
    flights_df: DataFrame, airports_df: DataFrame, airlines: DataFrame
) -> DataFrame:
    """
    ...
    flight_airport_df = flight_airport_df.join(
        ...
        F.col("destination_airport.latitude")
        .alias("destination_latitude")
        .cast(FloatType()),
        F.col("destination_airport.longitude")
    ...
    flights_df_airports_airlines = flight_airport_df.join(
        airlines, flight_airport_df.airline_icao == airlines.ICAO
    ).select(flight_airport_df["*"], airlines.Name.alias("airline_name"))

    print(
        f"{flights_df_airports_airlines.count()} flights after enriching with airlines"
    )
